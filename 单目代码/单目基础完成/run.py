...
META_SIZE = 12  # 4字节 label + 4字节 x + 4字节 y
SHM_SIZE = IMG_SIZE + META_SIZE
# 选择正确的串口设备
SERIAL_PORT = "/dev/ttyAMA0"  # 也可以尝试 /dev/ttyAMA0
BAUD_RATE = 9600

# 类别索引映射
class_names = ["Cone barrel"]
...
def get_depth_at(prediction, x, y, original_w, original_h):
    #  prediction.shape == (original_h, original_w)
    height, width = prediction.shape

    # 
    x_scaled = (x / original_w) * width
    y_scaled = (y / original_h) * height

    #map_coordinates 
    depth = map_coordinates(prediction, [[y_scaled], [x_scaled]], order=1, mode='nearest')[0]
    return depth
    ...
    if input_path is not None:
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                # 读取共享内存中的图像数据
                shm_data = memory.read(SHM_SIZE)
            
                # 提取图像部分
                frame = np.frombuffer(shm_data[:IMG_SIZE], dtype=np.uint8).reshape((HEIGHT, WIDTH, CHANNELS))
            
                # 提取YOLO检测数据
                label, x, y = struct.unpack("iii", shm_data[IMG_SIZE:IMG_SIZE + META_SIZE])
                
                # 获取类别名称
                obj_name = class_names[label] if 0 <= label < len(class_names) else "Unknown"
                #frame = video.read()
                
                msg = f"{x},{y}\n" 
                #ser.write(msg.encode('utf-8'))  # 发送字符串数据
                #print(f"发送坐标: {msg.strip()}")
                time.sleep(1)
                ...
                if frame is not None:
                    ...
                    if time.time()-time_start > 0:
                        ...
                        time_start = time.time()
                    print(f"\rFPS: {round(fps, 2)} | Object: {obj_name} at ({x}, {y})", end="")
                    
                    depth_value = get_depth_at(prediction, x, y, 640, 480)
                    print(f"Depth at ({x}, {y}): {depth_value*650}")
                    
                    #depth_value = prediction[x,y]*0.0015384  # 获取该点的深度值        
                    if depth_value>190 and depth_value<410:            
                        distance = f"1.5m\n"
                    if depth_value>670 :
                      distance = f"0.5m\n"
                    if depth_value>=410 and depth_value<=670:
                      distance = f"1.0m\n"                   
                    ser.write(distance.encode('utf-8'))  # 发送字符串数据
                    print(f"像素点 ({x}, {y}) 的相对深度值: {depth_value:.2f}m")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--model_weights',
                        default='/home/aono/Downloads/MiDaS/models/openvino_midas_v21_small_256.xml',
                        help='Path to the trained weights of model'
                        )

    parser.add_argument('-t', '--model_type',
                        default='openvino_midas_v21_small_256',
                        help='Model type: '
